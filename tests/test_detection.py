...
from adcs_lens.model import CertAuthority, CertLifecycle, Crl, Estate, Manifest

NOW = datetime(2026, 6, 15, 12, 0, 0, tzinfo=UTC)


def _ca(name: str, *, kind: str = "issuing", edit_flags: tuple[str, ...] = (),
        certs: tuple[CertLifecycle, ...] = ()) -> CertAuthority:
    return CertAuthority(
        name=name, dns="", config_string=f"host\\{name}", kind=kind,
        edit_flags=frozenset(edit_flags), interface_flags=frozenset(),
        audit_filter=None, validity="", roles=frozenset(), security=(), certs=certs,
    )


def _estate(*, cas: tuple[CertAuthority, ...] = (), crls: tuple[Crl, ...] = (),
            certs_parsed: bool = True) -> Estate:
    manifest = Manifest(
        collector_version="t", collected_at="", host="", domain="",
        skipped_passes=(), certs_parsed=certs_parsed,
    )
...
def test_expired_ca_cert_critical() -> None:
    cert = CertLifecycle("CN=Old", "issuing_ca", NOW - timedelta(days=800),
                         NOW - timedelta(days=1), "sha256", 2048)
...
def test_issuing_cert_near_expiry_high() -> None:
    cert = CertLifecycle("CN=Soon", "issuing_ca", NOW, NOW + timedelta(days=30), "sha256", 2048)
...
def test_root_cert_near_expiry_escalated_to_critical() -> None:
    cert = CertLifecycle("CN=Root", "root_ca", NOW, NOW + timedelta(days=30), "sha256", 4096)
    findings = detect_infra_cert_expiry(
        _estate(cas=(_ca("Root", kind="root", certs=(cert,)),)), now=NOW
    )
    assert findings[0].severity == "critical"
    assert findings[0].tier == "root"


def test_healthy_cert_no_finding() -> None:
    cert = CertLifecycle("CN=Fine", "issuing_ca", NOW, NOW + timedelta(days=1000), "sha256", 2048)
...
def test_expired_root_crl_critical_and_root_tier() -> None:
    crl = Crl("CN=Root", NOW - timedelta(days=400), NOW - timedelta(days=35), "root", "published")
...
def test_fresh_crl_no_finding() -> None:
    crl = Crl("CN=Iss", NOW - timedelta(days=2), NOW + timedelta(days=5), "issuing", "host")
